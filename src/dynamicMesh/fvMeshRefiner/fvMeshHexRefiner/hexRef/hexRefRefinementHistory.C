...
namespace Foam
{
    defineTypeNameAndDebug(hexRefRefinementHistory, 0);
}


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::hexRefRefinementHistory::writeEntry
(
...
void Foam::hexRefRefinementHistory::writeDebug
(
...
Foam::hexRefRefinementHistory::splitCell8::splitCell8()
:
    parent_(-1),
...
Foam::hexRefRefinementHistory::splitCell8::splitCell8(const label parent)
:
    parent_(parent),
    addedCellsPtr_(nullptr)
{}
...
Foam::hexRefRefinementHistory::splitCell8::splitCell8(Istream& is)
{
    is >> *this;
}


Foam::hexRefRefinementHistory::splitCell8::splitCell8(const splitCell8& sc)
:
    parent_(sc.parent_),
    addedCellsPtr_
    (
...
bool Foam::hexRefRefinementHistory::splitCell8::operator==(const splitCell8& s) const
{
...
Foam::Istream& Foam::operator>>(Istream& is, hexRefRefinementHistory::splitCell8& sc)
{
...
(
    Ostream& os,
    const hexRefRefinementHistory::splitCell8& sc
)
{
...
void Foam::hexRefRefinementHistory::checkIndices() const
{
    // Check indices.
...
Foam::label Foam::hexRefRefinementHistory::allocateSplitCell
(
    const label parent,
    const label i
)
...
void Foam::hexRefRefinementHistory::freeSplitCell(const label index)
{
...
void Foam::hexRefRefinementHistory::markSplit
(
    const label index,
    labelList& oldToNew,
...
void Foam::hexRefRefinementHistory::mark
(
    const label val,
    const label index,
...
Foam::label Foam::hexRefRefinementHistory::markCommonCells
(
...
void Foam::hexRefRefinementHistory::add
(
...
) const
{
    ...
    if (hexRefRefinementHistory::debug)
    {
...
void Foam::hexRefRefinementHistory::apply
(
...
Foam::hexRefRefinementHistory::hexRefRefinementHistory(const IOobject& io)
:
    regIOobject(io),
    active_(false)
{
    ...
    // that some processors do have hexRefRefinementHistory and some don't so
    // test for active has to be outside of above condition.
    active_ = (returnReduce(visibleCells_.size(), sumOp<label>()) > 0);

    if (debug)
    {
        Pout<< "hexRefRefinementHistory::hexRefRefinementHistory :"
...
Foam::hexRefRefinementHistory::hexRefRefinementHistory
(
    const IOobject& io,
    const List<splitCell8>& splitCells,
    const labelList& visibleCells,
...
(
    const IOobject& io,
    const hexRefRefinementHistory& rh
)
:
    regIOobject(io),
    active_(rh.active_),
    ...
    visibleCells_(rh.visibleCells())
{
    if (debug)
    {
        Pout<< "hexRefRefinementHistory::hexRefRefinementHistory : constructed initial"
            << " history." << endl;
    }
}
...
(
    ...
    const UPtrList<const hexRefRefinementHistory>& refs
)
:
    regIOobject(io),
    active_(false)
...
Foam::hexRefRefinementHistory::hexRefRefinementHistory(const IOobject& io, Istream& is)
:
    regIOobject(io),
    splitCells_(is),
    freeSplitCells_(0),
...
Foam::autoPtr<Foam::hexRefRefinementHistory> Foam::hexRefRefinementHistory::clone
(
    const IOobject& io,
...
) const
{
    ...
    return autoPtr<hexRefRefinementHistory>
    (
...
void Foam::hexRefRefinementHistory::resize(const label size)
{
    label oldSize = visibleCells_.size();

    if (debug)
    {
        Pout<< "hexRefRefinementHistory::resize from " << oldSize << " to " << size
            << " cells" << endl;
    }
...
void Foam::hexRefRefinementHistory::updateMesh(const mapPolyMesh& map)
{
    if (active())
    {
        ...
        if (debug)
        {
            Pout<< "hexRefRefinementHistory::updateMesh : from "
                << visibleCells_.size()
...
void Foam::hexRefRefinementHistory::subset
(
...
void Foam::hexRefRefinementHistory::countProc
(
    const label index,
...
void Foam::hexRefRefinementHistory::distribute(const mapDistributePolyMesh& map)
{
    ...
    //Pout<< "hexRefRefinementHistory::distribute :"
...
void Foam::hexRefRefinementHistory::compact()
{
    if (debug)
    {
        Pout<< "hexRefRefinementHistory::compact() Entering with:"
    ...
    if (debug)
    {
        Pout<< "hexRefRefinementHistory::compact : compacted splitCells from "
    ...
    if (debug)
    {
        Pout<< "hexRefRefinementHistory::compact() NOW:"
            << " freeSplitCells_:" << freeSplitCells_.size()
            << " splitCells_:" << splitCells_.size()
...
void Foam::hexRefRefinementHistory::writeDebug() const
{
    writeDebug(visibleCells_, splitCells_);
}


void Foam::hexRefRefinementHistory::storeSplit
(
    const label celli,
...
void Foam::hexRefRefinementHistory::combineCells
(
...
bool Foam::hexRefRefinementHistory::read()
{
...
bool Foam::hexRefRefinementHistory::readData(Istream& is)
{
    is >> *this;
    return !is.bad();
}
...
bool Foam::hexRefRefinementHistory::writeData(Ostream& os) const
{
    os << *this;
...
Foam::Istream& Foam::operator>>(Istream& is, hexRefRefinementHistory& rh)
{
...
Foam::Ostream& Foam::operator<<(Ostream& os, const hexRefRefinementHistory& rh)
{
    const_cast<hexRefRefinementHistory&>(rh).compact();
